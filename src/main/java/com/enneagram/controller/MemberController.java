...
public class MemberController {
	...
	public ResponseEntity<String> mailSending(HttpServletRequest request, String email,
		...
		String setfrom = "phantom_ysy@naver.com";
	...
	public void passwordChange_ok(MemberVO member, Model m, HttpServletRequest request, HttpServletResponse response)
			throws IOException {
		MemberVO i_member = memberSerivce.getMemberVO(member.getMno());
		i_member.setPassword(member.getPassword());
		memberSerivce.memberUpdate(i_member);
	...
	public void passwordChange(String email, Model m) throws IOException {
	...
	public void is_member(String kind, Model m) {
		m.addAttribute("kind", kind);
	}
	...
	public String callback(String state, String code, HttpServletRequest request, HttpServletResponse response, Model m)
			throws IOException {
	...
	public void member_info_check_ok(String password, String kind, HttpServletRequest request,
		...
		if (kind == "update") {
			...
			if (memberVO.getPassword().equals(password)) {
				memberSerivce.memberDeleteByMno(memberVO.getMno());
				out.print("<script>");
				out.print("alert('삭제되었습니다');");
				out.print("location.href='"+request.getContextPath()+"';"); // 컨텍스트루트...
				out.print("</script>");
			} else {
				out.print("<script>");
				out.print("alert('비밀번호가 같지 않습니다');");
				out.print("location.href='"+request.getContextPath()+"/member/myPage';");
				out.print("</script>");
			}
		}

		/*
		 * 여기부터시작 if(memberVO.getPassword().equals(password)) { out.print("<script>");
		 * out.print("location.href='/enneagram/member/member_info';"); // 컨텍스트루트...
		 * out.print("</script>"); }else { out.print("<script>");
		 * out.print("alert('비밀번호가 같지 않습니다');");
		 * out.print("location.href='/enneagram/member/myPage';");
		 * out.print("</script>"); }
		 */
